...
MessageCommon *createMessage_SendOutPutADC()
{
  ...
  message->byte0 = (uint8_t)(adc_reading1 >> 8);   // horních 8 bitů z value1
  message->byte1 = (uint8_t)(adc_reading1 & 0xFF); // spodních 8 bitů z value1
  message->byte2 = (uint8_t)(adc_reading2 >> 8);   // horních 8 bitů z value2
  message->byte3 = (uint8_t)(adc_reading2 & 0xFF); // spodních 8 bitů z value2
  message->byte4 = (uint8_t)(adc_reading3 >> 8);   // horních 8 bitů z value3
  message->byte5 = (uint8_t)(adc_reading3 & 0xFF); // spodních 8 bitů z value3
  message->byte6 = 0;
  message->byte7 = 0;
  return message;
}
...
MessageCommon *createMessage_SendInputADC()
{
  ...
  MessageCommon *message = new MessageCommon;
  message->byte0 = (uint8_t)(adc_reading4 >> 8);   // horních 8 bitů z value1
  message->byte1 = (uint8_t)(adc_reading4 & 0xFF); // spodních 8 bitů z value1
  message->byte2 = (uint8_t)(adc_reading5 >> 8);   // horních 8 bitů z value2
  message->byte3 = (uint8_t)(adc_reading5 & 0xFF); // spodních 8 bitů z value2
  message->byte4 = (uint8_t)(adc_reading6 >> 8);   // horních 8 bitů z value3
  message->byte5 = (uint8_t)(adc_reading6 & 0xFF); // spodních 8 bitů z value3
  message->byte6 = 0;
  message->byte7 = 0;
  return message;
}
...
{
  uint16_t *values = new uint16_t[3];
...
void handleCANMessage(int id, twai_message_t message)
{
  ...
  uint16_t *values = extractValuesFromCANMessage(data);
  switch (id)
  {
  case PWM_TARGET_ID:
